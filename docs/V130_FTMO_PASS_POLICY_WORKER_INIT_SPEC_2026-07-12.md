...
The worker-initialized one-path result matched the direct reference byte for
byte for E1. A cProfile execution of one path across C0/C1/P1 then measured
186.310161 seconds and 18,320,519 calls; `simulate_two_phase_path` consumed
176.863 seconds and `check_rails` was called 564,300 times. No 500-path
checkpoint or MC outcome was produced. The run was stopped as computationally
infeasible. This is an execution failure and tests no policy outcome.
