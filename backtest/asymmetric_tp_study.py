"""ASSET-CLASS ASYMMETRIC TP STUDY: crypto and index majors may have different
right-tail distributions, so test fixed class-specific take-profit distances vs the
v1.23 pure-bracket TP3 baseline.

PRE-REGISTERED cells (3, fixed before running):
  * crypto TP4 / indices TP3
  * crypto TP3 / indices TP3.5
  * crypto TP4 / indices TP3.5

No universe selection, no hold16, and no conditional hold. Protocol mirrors
exit_ladder_study.py: real per-instrument Deriv cost, stitched-OOS quarters, paired
per-signal t-stat, DSR deflated for 68 cumulative trials, 2x cost stress, stability,
and win-size metrics.
"""
...
CELLS = [("crypto4/index3", 4.0, 3.0),
         ("crypto3/index3.5", 3.0, 3.5),
         ("crypto4/index3.5", 4.0, 3.5)]


def tp_for(sym, crypto_tp, index_tp):
    return crypto_tp if sym in CRYPTO else index_tp


def tape(data, costs, crypto_tp=3.0, index_tp=3.0, block=True, cost_mult=1.0):
    recs = []
    for sym, df in data.items():
        c = costs[sym] * cost_mult
        if not np.isfinite(c):
            continue
        p = CParams(**{**BRACKET, "tp_atr": tp_for(sym, crypto_tp, index_tp)},
...
def main():
    ...
    print("ASSET-CLASS ASYMMETRIC TP STUDY — v1.23 bracket baseline, 12 majors, real cost")
    ...
    print(f"BASELINE (all tp3.00) OOS: N={bs['n']} exp={bs['exp']:+.4f} t={bs['t']:+.2f} "
    ...
    hdr = (f"{'cell':18s}{'N':>6s}{'exp':>8s}{'dExp':>8s}{'pair_t':>7s}{'WFE':>6s}"
           f"{'avgWin':>7s}{'medWin':>7s}{'>=2R%':>7s}{'exp2x':>8s}{'Qpos':>6s}{'Sym+':>6s}{'DSR':>6s}  VERDICT")
    print(hdr); print("-" * len(hdr))
    for label, ctp, itp in CELLS:
        t_blk = tape(data, costs, ctp, itp)
        ...
        cand_pair = split(tape(data, costs, ctp, itp, block=False))[1].set_index(["sym", "sig_i"])["r"]
        pt = pair_t(base_pair, cand_pair)
        e2 = stt(split(tape(data, costs, ctp, itp, cost_mult=2.0))[1]["r"].to_numpy())["exp"]
        ...
        print(f"{label:18s}{so['n']:6d}{so['exp']:+8.4f}{de:+8.4f}{pt:+7.2f}{wfe:6.2f}"
