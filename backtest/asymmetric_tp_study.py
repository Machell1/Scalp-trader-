"""ASYMMETRIC TP STUDY — different TP multiples for crypto vs index symbols.

Hypothesis: crypto and index CFDs have different microstructure / expansion profiles;
a single 3 ATR TP may be suboptimal for one class. Uses per-symbol tp_atr in the
runner (no EA change) — crypto gets a wider or tighter TP, indices keep baseline.

PRE-REGISTERED cells (4):
  crypto/index TP pairs: (3.5, 3.0), (4.0, 3.0), (3.5, 2.5), (3.0, 2.5).

Baseline = v1.23 pure bracket (tp 3.0 both classes). Protocol identical to exit_ladder_study.
"""
...
from walkforward_dsr import load_spreadgated, real_cost_per_side, SPREAD_GATED
...
CANDIDATES = [
    ("crypto3.5 / index3.0", 3.5, 3.0),
    ("crypto4.0 / index3.0", 4.0, 3.0),
    ("crypto3.5 / index2.5", 3.5, 2.5),
    ("crypto3.0 / index2.5", 3.0, 2.5),
]
N_TRIALS = 68 + len(CANDIDATES)


def tp_for(sym, tp_crypto, tp_index):
    return tp_crypto if sym in CRYPTO else tp_index


def collect(data, costs, tp_crypto, tp_index, block=True, cost_mult=1.0):
    recs = []
    for sym, df in data.items():
        c = costs[sym] * cost_mult
        if not np.isfinite(c):
            continue
        p = CParams(**{**BRACKET, "tp_atr": tp_for(sym, tp_crypto, tp_index)},
                    cost_atr_frac=c, block_overlap=block)
...
def main():
    ...
    print(f"ASYMMETRIC TP STUDY — {len(data)} majors ({len(CRYPTO)} crypto), "
          f"real cost, DSR N={N_TRIALS}\n")

    base_blk = collect(data, costs, 3.0, 3.0)
    _, base_oos = q_split(base_blk)
    bs_oos = stt(base_oos["r"].to_numpy())
    base_prd_oos = q_split(collect(data, costs, 3.0, 3.0, block=False))[1].set_index(["sym", "sig_i"])["r"]
    bwm = win_metrics(base_oos["r"])
    b2x = stt(q_split(collect(data, costs, 3.0, 3.0, cost_mult=2.0))[1]["r"].to_numpy())["exp"]
    print(f"BASELINE (symmetric tp3) OOS: N={bs_oos['n']} exp={bs_oos['exp']:+.4f} "
    ...
    for label, tp_c, tp_i in CANDIDATES:
        blk = collect(data, costs, tp_c, tp_i)
        ...
        prd = q_split(collect(data, costs, tp_c, tp_i, block=False))[1].set_index(["sym", "sig_i"])["r"]
        ...
        exp2 = stt(q_split(collect(data, costs, tp_c, tp_i, cost_mult=2.0))[1]["r"].to_numpy())["exp"]
