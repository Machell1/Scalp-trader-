"""PYRAMID-ADD STUDY — one scale-in at +R with stop moved to breakeven.

Hypothesis: winners that reach +1R quickly may deserve a single add with the
combined position protected at the original entry (stop-to-BE), lifting tail
capture without the ladder's early truncation.

PRE-REGISTERED cells (3):
  add 50% at +1.0R; add 50% at +1.5R; add 100% at +1.0R (all stop-to-BE).
...
CANDIDATES = [
    ("add 50% at +1.0R BE", 1.0, 0.5),
    ("add 50% at +1.5R BE", 1.5, 0.5),
    ("add 100% at +1.0R BE", 1.0, 1.0),
]
N_TRIALS = 68 + len(CANDIDATES)


def mkp(cost, block, add_r, add_frac, cost_mult=1.0):
    return CParams(**BRACKET, cost_atr_frac=cost * cost_mult, block_overlap=block,
                   pyramid_add_r=add_r, pyramid_add_frac=add_frac)


def collect(data, costs, add_r, add_frac, block=True, cost_mult=1.0):
    recs = []
    for sym, df in data.items():
        c = costs[sym] * cost_mult
        if not np.isfinite(c):
            continue
        tr, _ = simulate_symbol_c(df, mkp(c, block, add_r, add_frac), 0, len(df))
...
def main():
    ...
    print(f"PYRAMID-ADD STUDY — {len(data)} majors, real cost, DSR N={N_TRIALS}\n")
    ...
    _, base_oos = q_split(base_collect())
    ...
    for label, add_r, add_frac in CANDIDATES:
        blk = collect(data, costs, add_r, add_frac)
        ...
        prd = q_split(collect(data, costs, add_r, add_frac, block=False))[1].set_index(["sym", "sig_i"])["r"]
        ...
        exp2 = stt(q_split(collect(data, costs, add_r, add_frac, cost_mult=2.0))[1]["r"].to_numpy())["exp"]
