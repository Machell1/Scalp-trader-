"""PURE-BRACKET TP STUDY: does a single wider fixed take-profit raise win size or
expectancy vs the v1.23 pure bracket without relying on hold16?

PRE-REGISTERED cells (3, fixed before running):
  * pure bracket TP 3.25 / 3.50 / 4.00 ATR, max hold 8 bars.

Baseline = v1.23 live pure bracket (SL 1 ATR / TP 3 ATR / hold 8, no lock/trail).
Protocol mirrors exit_ladder_study.py: real per-instrument Deriv cost, calendar-quarter
stitched OOS, paired per-signal t-stat, DSR deflated for the cumulative 68 trials,
2x cost stress, quarter/symbol stability, and win-size metrics.
"""
...
CELLS = [("tp3.25 h8", 3.25), ("tp3.50 h8", 3.50), ("tp4.00 h8", 4.00)]


def tape(data, costs, tp_atr, block=True, cost_mult=1.0):
    recs = []
    for sym, df in data.items():
        c = costs[sym] * cost_mult
        if not np.isfinite(c):
            continue
        p = CParams(**{**BRACKET, "tp_atr": tp_atr}, cost_atr_frac=c, block_overlap=block)
...
def main():
    ...
    print("PURE-BRACKET TP STUDY — v1.23 bracket baseline, 12 majors, real cost")
    print(f"DSR deflation: {N_TRIALS} cumulative research trials\n")

    base = tape(data, costs, BRACKET["tp_atr"])
    base_is, base_oos = split(base)
    bs = stt(base_oos["r"].to_numpy())
    baw, bmw, b2 = wins(base_oos["r"])
    base_pair = split(tape(data, costs, BRACKET["tp_atr"], block=False))[1].set_index(["sym", "sig_i"])["r"]
    base2x = stt(split(tape(data, costs, BRACKET["tp_atr"], cost_mult=2.0))[1]["r"].to_numpy())["exp"]
    print(f"BASELINE (tp3.00 h8) OOS: N={bs['n']} exp={bs['exp']:+.4f} t={bs['t']:+.2f} "
    ...
    for label, tp in CELLS:
        t_blk = tape(data, costs, tp)
        ...
        cand_pair = split(tape(data, costs, tp, block=False))[1].set_index(["sym", "sig_i"])["r"]
        pt = pair_t(base_pair, cand_pair)
        e2 = stt(split(tape(data, costs, tp, cost_mult=2.0))[1]["r"].to_numpy())["exp"]
