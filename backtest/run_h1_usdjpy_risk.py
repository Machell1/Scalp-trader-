"""Run the preregistered H1 USDJPY dynamic-risk sleeve study."""
...
RESULT = HERE / "h1_usdjpy_risk_results.json"
SPEC = HERE.parent / "docs" / "H1_USDJPY_RISK_SPEC_2026-07-13.md"
...
def policy_for(symbols: tuple[str, ...], name: str, usdjpy_risk: float) -> RiskPolicy:
    mapping = {symbol: (usdjpy_risk if symbol == "USDJPY" else 0.0030) for symbol in symbols}
...
def evaluate(
    ...
    for policy in policies:
        ...
        risk = dict(policy.phase1)["USDJPY"]
        row = {
            "policy": policy.name,
            "usdjpy_risk_fraction": risk,
            "usdjpy_risk_percent": 100.0 * risk,
            "summary": summary,
            "paired": comparison,
            "pass": passed,
            "failures": failures,
...
def run_stage(
    ...
    control_policy = policy_for(tuple(metas), "CONTROL_030", 0.0030)
...
def main() -> None:
    control_tape, control_counts = build_h1_universe_tape(BASE_SOURCES, stress=True)
    ...
    policies = tuple(
        policy_for(symbols, f"USDJPY_{int(risk * 10000):03d}", risk) for risk in RISKS
    )
    ...
    selected = sorted(
        passing,
        key=lambda row: (-row["summary"]["both_probability"], row["usdjpy_risk_fraction"]),
    )[0]
    ...
    output["verdict"] = "ADMIT_USDJPY" if confirmed else "NO_ADMISSION"
    write_result(output)
