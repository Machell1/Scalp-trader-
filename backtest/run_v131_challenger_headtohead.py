"""Run the preregistered v1.31 control versus E3 exit confirmation."""
...
ROOT = HERE.parent
SPEC = ROOT / "docs" / "V131_CHALLENGER_HEADTOHEAD_SPEC_2026-07-16.md"
SPEC_HASH_FILE = SPEC.with_suffix(".sha256")
RESULT = HERE / "v131_challenger_headtohead_results.json"
...
GATES = {
    "both_strictly_greater": 0.854740,
    "wilson_lower_strictly_greater": 0.852898,
    "hard_no_greater_than": 0.003700,
    "timeout_strictly_less": 0.141560,
...
def gate_result(candidate_summary: dict, comparison: dict) -> tuple[bool, list[str]]:
    failures = []
    if candidate_summary["both_probability"] <= GATES["both_strictly_greater"]:
        failures.append("BOTH_NOT_STRICTLY_GT_85_4740PCT")
    if candidate_summary["both_wilson_lower"] <= GATES["wilson_lower_strictly_greater"]:
        failures.append("WILSON_LOWER_NOT_STRICTLY_GT_85_2898PCT")
    if candidate_summary["hard_probability"] > GATES["hard_no_greater_than"]:
        failures.append("HARD_GT_0_3700PCT")
    if candidate_summary["timeout_probability"] >= GATES["timeout_strictly_less"]:
        failures.append("TIMEOUT_NOT_STRICTLY_LT_14_1560PCT")
...
def run_stage(control_tape, candidate_tape, metas, policy, boot, paths, label) -> dict:
    control = run_chunks(
        control_tape, metas, policy, boot, paths, f"{label}:V131_CONTROL"
    )
    candidate = run_chunks(
        candidate_tape, metas, policy, boot, paths, f"{label}:E3_CHALLENGER"
    )
...
def main() -> None:
    ...
    policy = RiskPolicy("V131_DYNAMIC_RISK", risk_map, risk_map)
    ...
    code_files = (
        ...
        HERE / "run_v131_challenger_headtohead.py",
        ...
        ROOT / "mql5" / "MomentumPullbackV131ChallengerEA.mq5",
    )
    output = {
        "verdict": "INCOMPLETE",
    ...
    if not output["screen"]["pass"]:
        output["verdict"] = "NO_CONFIRMATION_PASS"
        write_result(output)
        print("FINAL NO_CONFIRMATION_PASS screen gate failed", flush=True)
    ...
    output["verdict"] = (
        "CONFIRMATION_PASS"
        if output["confirmation"]["pass"] else "NO_CONFIRMATION_PASS"
    )
    write_result(output)
