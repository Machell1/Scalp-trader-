"""STOP-BUFFER STUDY: the v1.23 pure bracket uses a 1 ATR stop. Test whether a
modestly wider initial stop reduces noise stop-outs enough to improve net expectancy.

PRE-REGISTERED cells (3, fixed before running):
  * stop 1.10 / 1.25 / 1.50 ATR, TP remains 3 ATR, hold remains 8 bars.

This is a risk-geometry study, not a filter and not a new exit ladder. Baseline =
v1.23 pure bracket. Protocol mirrors exit_ladder_study.py: real per-instrument Deriv
cost, stitched-OOS quarters, paired per-signal t-stat, DSR deflated for 68 cumulative
trials, 2x cost stress, stability, and win-size metrics.
"""
...
CELLS = [("stop1.10", 1.10), ("stop1.25", 1.25), ("stop1.50", 1.50)]


def tape(data, costs, stop_atr=1.0, block=True, cost_mult=1.0):
    recs = []
    for sym, df in data.items():
        c = costs[sym] * cost_mult
        if not np.isfinite(c):
            continue
        p = CParams(**{**BRACKET, "stop_atr": stop_atr}, cost_atr_frac=c, block_overlap=block)
...
def main():
    ...
    print("STOP-BUFFER STUDY — v1.23 bracket baseline, 12 majors, real cost")
    ...
    print(f"BASELINE (stop1.00) OOS: N={bs['n']} exp={bs['exp']:+.4f} t={bs['t']:+.2f} "
    ...
    hdr = (f"{'cell':10s}{'N':>6s}{'exp':>8s}{'dExp':>8s}{'pair_t':>7s}{'WFE':>6s}"
           f"{'avgWin':>7s}{'medWin':>7s}{'>=2R%':>7s}{'exp2x':>8s}{'Qpos':>6s}{'Sym+':>6s}{'DSR':>6s}  VERDICT")
    print(hdr); print("-" * len(hdr))
    for label, stop in CELLS:
        t_blk = tape(data, costs, stop)
        ...
        cand_pair = split(tape(data, costs, stop, block=False))[1].set_index(["sym", "sig_i"])["r"]
        pt = pair_t(base_pair, cand_pair)
        e2 = stt(split(tape(data, costs, stop, cost_mult=2.0))[1]["r"].to_numpy())["exp"]
        ...
        print(f"{label:10s}{so['n']:6d}{so['exp']:+8.4f}{de:+8.4f}{pt:+7.2f}{wfe:6.2f}"
