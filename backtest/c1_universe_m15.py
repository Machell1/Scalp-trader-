"""M15-grain tape builder for the preregistered C1 universe-admission study.
...
class C1M15Execution:
    ...
    def resolve(
        ...
        if exit_bar is None:
            ...
            # C1-era parity fix (spec 3166c4f, registered sha 796f2450): exit price
            # stays structural (bid close).  The old side<0 spread add-on double-
            # charged shorts (round-trip already in entry_cost_r) and made E1/E2
            # tapes differ structurally -> parity gate fired.
            reason = "TIME"
...
def build_tape(
    ...
    *,
    segment: str,
    cost_mult: float,
    momentum_atr_mult: float = MOMENTUM_C1,
) -> BuiltTape:
    ...
    if len(sources) > 1 and tuple(sources[:4]) != BASE_SOURCES:
        raise ValueError("current C1 priority must remain fixed at positions 0..3")
    ...
    execution = C1M15Execution(contexts_by_symbol, cost_mult)
