"""Synthetic fidelity checks for the v1.36-A1 M15 universe builder."""
...
def self_test() -> dict:
    ...
    long_data.atr[3] = 1.0
    execution = A1M15Execution({"LONG": long_context}, 2.0)
    long_data.l[4] = 9.90
    long_data.l[5] = 9.80
    ...
    short_data.h[4] = 9.9
    short_data.h[5] = 10.0
    short_exec = A1M15Execution({"SHORT": short_context}, 2.0)
    _check(
    ...
    stop_data.l[4] = 8.9
    stop_data.h[4] = 12.0
    stop_plan = A1M15Execution({"STOP": stop_context}, 2.0).resolve(
    ...
    win_data.l[4] = 9.5
    win_data.h[4] = 11.5
    win_plan = A1M15Execution({"WIN": win_context}, 2.0).resolve(
    ...
    ask_stop_plan = A1M15Execution({"ASKSTOP": ask_stop_context}, 2.0).resolve(
    ...
    short_win_plan = A1M15Execution({"SHORTWIN": short_win_context}, 2.0).resolve(
    ...
    time_data.atr[3] = 1.0
    time_plan = A1M15Execution({"TIME": time_context}, 2.0).resolve(
    ...
    short_time_plan = A1M15Execution({"SHORTTIME": short_time_context}, 2.0).resolve(
        short_time_data, 3, 4, -1, 10.0, 1.0
    )
    _check(
        "short_time_exit_uses_ask_close",
        short_time_plan.reason == "TIME"
        and math.isclose(short_time_plan.exit_price, 10.2, abs_tol=1e-12),
        passed,
    )

    gap_data, gap_context = _synthetic("GAP", 0, hours=12, weekend_gap=True)
    gap_data.atr[3] = 1.0
    gap_plan = A1M15Execution({"GAP": gap_context}, 2.0).resolve(
    ...
    run_live(
        ...
        execution=A1M15Execution({"PENDING": pending_context}, 2.0),
    ...
    run_live(
        ...
        caps=CAPS_SOURCE,
        window=PENDING_M15_BARS,
        execution=A1M15Execution(contexts, 2.0),
    ...
    for cost_mult in (1.0, 2.0):
        ...
        trades, _ = run_live(
            ...
            caps=CAPS_SOURCE,
            window=PENDING_M15_BARS,
            execution=A1M15Execution({"FLOW": flow_context}, cost_mult),
            event_sink=events.append,
        )
    ...
    run_live(
        ...
        caps=CAPS_SOURCE,
        window=PENDING_M15_BARS,
        execution=A1M15Execution({"NONW2": non_w2_context}, 2.0),
...
if __name__ == "__main__":
    result = self_test()
    print(f"V136_A1_UNIVERSE_SYNTHETIC passed={result['passed']}")
