"""Run the preregistered v1.31 control versus v1.33-C1 confirmation."""
...
ROOT = HERE.parent
SPEC = ROOT / "docs" / "V133_C1_CORRECTED_HEADTOHEAD_SPEC_2026-07-18.md"
SPEC_HASH_FILE = SPEC.with_suffix(".sha256")
RESULT = HERE / "v133_c1_headtohead_results.json"
...
def run_stage(control_tape, candidate_tape, metas, policy, boot, paths, label) -> dict:
    ...
    candidate = run_chunks(
        candidate_tape, metas, policy, boot, paths, f"{label}:V133_C1"
    )
...
def main() -> None:
    ...
    if common_blocks != EXPECTED_COMMON_BLOCKS:
        raise RuntimeError(
            f"common eligible block discrepancy: {common_blocks} "
            f"!= {EXPECTED_COMMON_BLOCKS}"
        )
    path0 = {
    ...
    code_files = (
        ...
        HERE / "run_v133_c1_headtohead.py",
    ...
    output = {
        ...
        "candidate_ea_implementation": "not supplied; geometry-only harness test",
    ...
    if not output["screen"]["pass"]:
        output["verdict"] = "C1_SCREEN_REJECTED"
        write_result(output)
        print("FINAL C1_SCREEN_REJECTED screen gate failed", flush=True)
    ...
    output["verdict"] = (
        "C1_CONFIRMED_BEATS_V131"
        if output["confirmation"]["pass"] else "C1_CONFIRMATION_REJECTED"
    )
    write_result(output)
