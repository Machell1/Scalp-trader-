"""Execute the preregistered v1.36-A1 M15-grain universe-admission study."""
...
from test_v136_a1_universe_admission import self_test as universe_self_test
from v136_a1_universe_m15 import (
    BASE_SOURCES,
    BASE_SYMBOLS,
    CANDIDATES,
    MOMENTUM_C1,
...
ROOT = HERE.parent
SPEC = ROOT / "docs" / "V136_A1_UNIVERSE_ADMISSION_SPEC_2026-07-18.md"
SPEC_HASH_FILE = SPEC.with_suffix(".sha256")
RESULT = HERE / "v136_a1_universe_admission_results.json"
FAILURES = HERE / "v136_a1_universe_admission_failures.jsonl"
CHECKPOINTS = ROOT.parent / ".v136_a1_universe_checkpoints"

DATA_OK = "verified 46 OK, 0 missing, 0 mismatched"
EXPECTED_SPEC_SHA = "665d90cd5b9e6d0644dd5cdc824115be460ff8ba896e53198d382d370507c119"
...
EXPECTED_LIVE_EA_SHA = "531d2a8b305e145241d00b9db11f716b49f1d90f190914046ce7b7cdae02b833"
EXPECTED_PARENT_COMMIT = "7c21153900b83e3253ee4c1713bf1913740b9b85"
EXPECTED_SPEC_COMMIT = "133834e6146c60d4ffe72cea1cf970938ee3743b"
...
EXPERIMENT_FILES = (
    SPEC,
    SPEC_HASH_FILE,
    ROOT / "docs" / "codex" / "V136_A1_UNIVERSE_IMPLEMENTATION_NOTE_2026-07-18.md",
    HERE / "run_v136_a1_universe_admission.py",
    HERE / "v136_a1_universe_m15.py",
    HERE / "test_v136_a1_universe_admission.py",
...
def verify_provenance() -> dict:
    ...
    if introduced != [EXPECTED_SPEC_COMMIT]:
...
def protected_regressions() -> dict:
    ...
    a1_tape, a1_counts = build_h1_universe_tape(
        BASE_SOURCES,
        stress=True,
        partial_fraction=0.75,
        target_atr=1.5,
    ...
    rows = {
        ...
        "a1": {
            "trades": len(a1_tape.trades), "events": len(a1_tape.events),
            "sha256": _h1_event_hash(a1_tape), "counts": a1_counts,
            "expected": {"trades": 662, "events": 2819, "sha256": A1_SHA},
        },
    }
...
def policy_for(symbols: tuple[str, ...]) -> RiskPolicy:
    ...
    return RiskPolicy("V136_A1_UNIVERSE_005", risk_map, risk_map)
...
def account_gate(candidate: dict, control: dict, comparison: dict, keep: dict) -> tuple[bool, list[str]]:
    ...
    if keep["overall_fraction"] < 0.97:
        failures.append("A1_FILL_RETENTION_LT_97PCT")
...
def account_stage(
    ...
    for cell in labels:
        ...
        keep = retention(builds["A1_CONTROL"], builds[cell])
...
def main() -> None:
    ...
    screen_builds = {"A1_CONTROL": control_first}
    sources_by_label = {"A1_CONTROL": BASE_SOURCES}
    symbol_by_cell = {}
    source_by_cell = {}
