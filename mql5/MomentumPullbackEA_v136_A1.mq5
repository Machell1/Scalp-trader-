//+------------------------------------------------------------------+
//|   v1.36-A1 CANDIDATE (2026-07-18): A+ SIGNAL THRESHOLD on the    |
//|     CONFIRMED v1.33-C1 geometry. One input rename (house          |
//|     convention: chart-saved values cannot survive):               |
//|       * InpMomentumAtrMultV136 = 3.0 (was 2.0): only trade        |
//|         impulses >= 3.0 ATR - the "A+ only" arm of the owner's    |
//|         v1.36 brief.                                              |
//|     Evidence (pre-registered 7-cell paired screen, 20,000 CRN     |
//|     paths, seed 13020260711, E2 stress, repo harness, baseline    |
//|     = confirmed C1): pass 93.22% vs 89.65% (+3.57pp, paired LB    |
//|     +2.82pp > 0), hard halt 0.015% <= 0.370% gate, timeout        |
//|     6.77% vs 10.35% (-3.58pp), trades -60% (395 vs 987),          |
//|     per-trade OOS +0.261R vs +0.252R. Fragility neighbors: 2.5    |
//|     also positive (+1.54pp LB), 3.5 negative (-0.35pp) - smooth   |
//|     interior peak at 3.0, not a knife-edge. Trade-off: median     |
//|     completion ~2x calendar days (895 vs 462) - FTMO has no       |
//|     time limit.                                                   |
//|     Sibling cells (all KILLED, published): A2 compression gate    |
//|     (LB -4.02pp), A3 lead-lag (LB -1.08pp, hard 3.45%), A4 A+    |
//|     + 1.0 ATR deep pullback (LB -6.65pp, throughput collapse),    |
//|     R5 5R residual w/ bank (LB -10.06pp), F5 raw 5R no bank       |
//|     (hard 55.85% - 3rd variance confirmation).                    |
//|     STATUS: CANDIDATE - NOT validated on the owner's corrected-   |
//|     fidelity harness. Do not deploy until the 100k paired         |
//|     confirmation + owner sign-off per house runbook.              |
//|     Everything below is v1.33-C1 unchanged.                       |
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
...
//|     STATUS: GEOMETRY CONFIRMED 2026-07-18 on the owner's          |
//|     corrected-fidelity harness, 100,000 paired E2-stress paths    |
//|     @748390c: pass 88.274% vs 76.823% (+11.451pp, paired LB       |
//|     +11.0265pp), hard halt 0.266% <= 0.370% gate, timeout         |
//|     11.460% vs 19.233%, median 572d vs 772d. Verdict:             |
//|     C1_CONFIRMED_BEATS_V131; 88% target cleared incl. Wilson LB   |
//|     88.106%. Remaining gate before install: EA diff-audit +       |
//|     compile (0/0) + owner sign-off per house runbook.             |
...
#property strict
#property description "Multi-symbol H1 momentum pullback EA v1.36-A1 CANDIDATE: A+ impulses >= 3.0 ATR on CONFIRMED v1.33-C1 geometry (bank 75% @ +1R, TP 1.5 ATR, pullback 0.6). NOT validated on the corrected-fidelity harness - do not deploy until the 100k confirmation + owner sign-off. USDJPY 0.05% sleeve; trio 0.30%; v1.32 fidelity fixes ON; arms B1-B3 OFF."
...
input double InpMomentumAtrMultV136 = 3.0;  // v1.36 A1 candidate: A+ impulses only (>= 3.0 ATR). Renamed so chart-saved 2.0 cannot survive. Revert to 2.0 to restore v1.33-C1 behavior
...
int OnInit()
  {
   ...
   PrintFormat("MomentumPullbackEA v1.36-A1 CANDIDATE ready. Entry=%s. Exits=%s + TP%.2f/time. ManageOnBarClose=%s. Scanning %d symbols on %s. Base risk=%.2f%%; USDJPY risk=%.2f%%. v1.32 arms: exit=%s stopEval=%s (defaults OFF = v1.31 behavior).",
